...
export class BuilderSchemaPublisher {
  async publish(schema: BuilderSchema, projectId: string): Promise<SchemaDto> {
    ...
    const { jsonSchema, jsonLdContext } = generate(
      ...
      this.attributesToFields(schema.attributes.filter(a => !a.parentId), schema.attributes),
    )
  ...
  private async generateNextVersion(input: { type: string; scope: 'public' | 'unlisted'; did: string; apiKeyHash: string }) {
