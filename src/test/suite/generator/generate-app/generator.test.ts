...
import { buildAppGenerateCommand, DIRECTORY_NAME_DUPLICATION_ERROR_MESSAGE, APP_SUCCESSFULLY_CREATED_MESSAGE } from "../../../../utils/cliHelper";
import { generateAffinidiAppWithCLI, NO_DIRECTORY_SELECTED_MESSAGE, NO_APP_NAME_SELECTED_MESSAGE } from "../../../../generators/create-app/generator";
...
describe("generateAffinidiAppWithCLI()", () => {
  ...
  it("should render app with specified params", async () => {
    ...
    expect(dialog).called;
    expect(ext.outputChannel.appendLine).calledWith(buildAppGenerateCommand(path.join(DIRECTORY_NAME, APP_NAME)));
