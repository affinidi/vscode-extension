...
export class AffinidiDevToolsProvider implements vscode.TreeDataProvider<AffResourceTreeItem>, vscode.TreeDragAndDropController<AffResourceTreeItem> {
  ...
  public getChildren(element?: AffResourceTreeItem): Thenable<AffResourceTreeItem[]> {
    ...
    switch (element?.resourceType)
    {
      ...
      case AffinidiVariantTypes[AffinidiVariantTypes.rootProjects]:
      {
        this.addNewTreeItem(
          treeNodes,
  ...
  public async handleDrag(source: AffResourceTreeItem[], treeDataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
  ...
  private getAppropriateDragObject(source: AffResourceTreeItem) : AffinidiVariant | undefined
  {
    return this.rawResources.find(item => item.affId === source.metadata.affId);
  }

  private addNewTreeItem(treeNodes:AffResourceTreeItem[], type:AffinidiVariantTypes, affId:string, label:string, description:string, state: vscode.TreeItemCollapsibleState, canDrag:boolean, icon: ThemeIcon = ThemeIcon.Folder)
  {
    treeNodes.push(new AffResourceTreeItem(
...
class AffResourceTreeItem extends vscode.TreeItem {
  ...
  private _onDidChangeTreeData: vscode.EventEmitter<AffResourceTreeItem | undefined | null | void> = new vscode.EventEmitter<AffResourceTreeItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<AffResourceTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
}
